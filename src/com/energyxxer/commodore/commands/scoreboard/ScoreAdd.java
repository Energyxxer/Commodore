...
public class ScoreAdd extends ScoreboardManipulation {
    ...
    public String getOperationContent(Entity sender) {
        return "scoreboard players " + ((delta < 0) ? "remove" : "addCriterion") + " " + CommandUtils.getRawReference(score.getParent().getHolder(), sender) + " " + score.getObjective().getName() + " " + Math.abs(delta);
    }
}
