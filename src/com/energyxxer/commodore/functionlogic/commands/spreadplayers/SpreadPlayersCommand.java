...
public class SpreadPlayersCommand implements Command {
    ...
    public SpreadPlayersCommand(@NotNull Entity targets, @NotNull CoordinateSet center, double spreadDistance, double maxRange, boolean respectTeams, int under) {
        this.center = center;
        this.spreadDistance = spreadDistance;
        this.maxRange = maxRange;
        this.respectTeams = respectTeams;
    ...
    public CommandResolution resolveCommand(ExecutionContext execContext) {
        String underClause = "";
        if(under != null) {
            underClause = "under " + under + " ";
        }
        return new CommandResolution(execContext, "spreadplayers " + center.getXZAs(Coordinate.DisplayMode.ENTITY_POS) + " " + CommandUtils.numberToPlainString(spreadDistance) + " " + CommandUtils.numberToPlainString(maxRange) + " " + underClause + respectTeams + " " + targets);
    }
    ...
    public void assertAvailable() {
        ...
        if(under != null) {
            VersionFeatureManager.assertEnabled("command.spreadplayers.under");
        }
    }
}
